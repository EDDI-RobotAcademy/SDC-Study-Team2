...
post_info *init_post_info_by_write_command(void)
{
    ...
    // post_info_title_from_request(tmp_post_info, title_from_keyboard);
    ...
    post_info_writer_from_request(tmp_post_info, writer_from_keyboard);
    
    printf("내용을 입력하세요.\n");
    get_user_keyboard_input(keyboard_input_from_user);
    char *contents_from_keyboard = keyboard_input_from_user;
    post_info_contents_from_request(tmp_post_info, contents_from_keyboard);

    //printf("title: %s\nwriter: %s\ncontents: %s\n", title_from_keyboard, writer_from_keyboard, contents_from_keyboard);
